#include <babylon/loading/plugins/gltf/2.0/gltf_loader.h>
...
std::vector<std::string> GLTFLoader::_RegisteredExtensions;
std::unordered_map<std::string, std::function<IGLTFLoaderExtensionPtr(GLTFLoader& loader)>>
  GLTFLoader::_RegisteredExtensionFactories;
  ...
  const std::function<IGLTFLoaderExtensionPtr(GLTFLoader& loader)>& factory)
{
  ...
  GLTFLoader::_RegisteredExtensionFactories[name] = factory;

  // Keep the order of registration so that extensions registered first are called first.
  GLTFLoader::_RegisteredExtensions.emplace_back(name);
}

bool GLTFLoader::UnregisterExtension(const std::string& name)
{
  if (!stl_util::contains(GLTFLoader::_RegisteredExtensionFactories, name)) {
    return false;
  }

  GLTFLoader::_RegisteredExtensionFactories.erase(name);
  stl_util::erase(GLTFLoader::_RegisteredExtensions, name);
...
{
  for (const auto& name : GLTFLoader::_RegisteredExtensions) {
    const auto& extension = GLTFLoader::_RegisteredExtensionFactories[name](*this);
    if (extension->name != name) {
      BABYLON_LOGF_WARN("GLTFLoader",
                        "The name of the glTF loader extension instance does not match the "
                        "registered name: %s != %s",
                        extension->name.c_str(), name.c_str())
    }

    _extensions[name] = extension;
    _parent.onExtensionLoadedObservable.notifyObservers(extension.get());
  }
...
INodePtr GLTFLoader::_createRootNode()
{
  ...
  switch (_parent.coordinateSystemMode) {
    ...
    default: {
      throw std::runtime_error(StringTools::printf("Invalid coordinate system mode %d",
                                                   static_cast<int>(_parent.coordinateSystemMode)));
    }
  }
...
std::vector<TransformNodePtr> GLTFLoader::_getTransformNodes()
{
  ...
  if (!nodes.empty()) {
    for (const auto& node : nodes) {
      if (node->_babylonTransformNode
          && node->_babylonTransformNode->getClassName() == "TransformNode") {
        transformNodes.emplace_back(node->_babylonTransformNode);
      }
    }
  }
                                                      ...
                                                      IAccessor& accessor, const std::string& kind)
{
  ...
  // on the fly.
    ...
    accessor._babylonVertexBuffer = std::make_unique<VertexBuffer>(
      _babylonScene->getEngine(), babylonBuffer, kind, false, false, bufferView.byteStride, false,
